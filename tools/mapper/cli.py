...
and reports which of them the walk reached (#255), `sweeps` runs the completeness challenge the
protocol requires over the units the inventory left unaccounted (#250), and `stage` produces the
inputs a blind second mapping is given -- the documents with every worked example drawn from the
corpus under mapping removed -- and, with `--verify`, holds a committed run to the digests of
what it was given (#223). Producing a map is still done by hand (docs/method.md); what this makes
checkable is that the walk was the walk this corpus requires, how much of the extent it covered,
what is left in the part it did not account for, and that a second walk was blind.
