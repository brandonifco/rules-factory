...
class TestRecord(ProvenanceCase):
    ...
    def test_the_fields(self):
        ...
        self.assertEqual(record["map"]["version"], "2.0.0")
...
class TestRecompute(ProvenanceCase):
    ...
    def test_a_pin_edited_back_after_a_map_upgrade(self):
        ...
        v1, v2 = pack_version(PART107, "3.0.0", root), pack_version(PART107, "4.0.0", root)
        out = self.produced(package=v1)
        self.produced(out=out, package=v2)
        self.assertEqual(self.record(out)["map"]["version"], "4.0.0")
        code, output = self.recompute(out, package=v2)
        self.assertEqual(code, 0, output)

        props = pathlib.Path(out, PACKAGES_PROPS)
        text = props.read_text(encoding="utf-8")
        pin = f'<PackageVersion Include="{MAP_ID}" Version="[4.0.0]" />'
        self.assertIn(pin, text)
        props.write_text(text.replace(pin, pin.replace("4.0.0", "3.0.0")), encoding="utf-8")
    ...
    def test_a_pin_edited_back_after_a_map_downgrade(self):
        ...
        self.assertEqual(code, 0, output)
        props = pathlib.Path(out, PACKAGES_PROPS)
        props.write_text(props.read_text(encoding="utf-8").replace("[3.0.0]", "[4.0.0]"), encoding="utf-8")
