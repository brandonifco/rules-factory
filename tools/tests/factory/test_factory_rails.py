...
class TestTheReviewPacket(RailsInAGitEngine):
    ...
    def test_packet_material_comes_from_the_reviewed_head_not_the_caller_checkout(self):
        ...
        self.commit_engine()
        git(self.out, "checkout", "-qb", "reviewed-head")
        path = os.path.join(self.out, "provenance.json")
        record = json.load(open(path, encoding="utf-8"))
        record["map"]["version"] = "334-reviewed-head"
        with open(path, "w", encoding="utf-8") as handle:
            json.dump(record, handle, indent=2)
            handle.write("\n")
        git(self.out, "add", "provenance.json")
        git(self.out, "commit", "-qm", "make reviewed head provenance distinct")
        head = git(self.out, "rev-parse", "HEAD")

        # The caller is deliberately back on A while GitHub reports B as the PR head.
        git(self.out, "checkout", "-q", "main")
        self.pull_request(head)
        text = self.rendered()
