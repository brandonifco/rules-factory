...
all of them, so two entry branches collided on twenty files neither was about. So:
  ...
  * `create` renders the items in memory, from the same two inputs `produce` used -- the map
    package `provenance.json` records, merged with the engine's `corpus-map.overlay.json`
...
def render(entries, context):
    ...
    index = [f"# Backlog for {context['name']}\n\n",
             ...
             "`corpus-map.overlay.json` by `factory backlog`; not a file in the engine.\n\n"]
...
def engine_backlog(engine_dir, package=None):
    ...
    record hashed) and the engine's own `corpus-map.overlay.json`, merged under decision 0015 by
    ...
    overlay_path = os.path.join(engine_dir, generate.OVERLAY_NAME)
    overlay = {}
    if os.path.isfile(overlay_path):
        try:
            ...
            raise BacklogError(f"cannot read {overlay_path}: {error}")
    try:
        merged = generate.merge(document_map, overlay, root=engine_dir)
    except generate.GenerationError as error:
        raise BacklogError(f"the map and {generate.OVERLAY_NAME} do not merge, so there is no backlog to "
                           f"render: {error}")
