...
def refuse_quotes(engine_dir, files, package=None):
    """Decision 0022: nothing quoting a licensed local-copy corpus is sent to GitHub.

    Only for an engine whose provenance.json records `licensedCopyException`. The map is read from
    `package` (a .nupkg path or Id@Version) or else Id@Version from provenance.json, taken from a
    file or the NuGet global packages folder and never downloaded (a local-copy map is never
    published), and must be the map provenance hashed. Every body must carry WITHHELD and contain
    none of `quoted_strings`. Returns the number of strings compared, or None for any other engine.
    """
    ...
    try:
        ...
        raise BacklogError(f"cannot read {engine_dir}/provenance.json, so whether the corpus is licensed is "
                           f"unknown: {error}")
    if not isinstance(record, dict) or "licensedCopyException" not in record:
        return None
    ...
    if os.path.isfile(spec):
        ...
        if not os.path.isfile(nupkg):
            raise BacklogError(f"the engine was produced from a licensed local-copy corpus (0022), and its map "
                               f"package {spec} is not a local file or in the NuGet global packages folder, so "
                               f"the issue bodies cannot be checked for its text; pass --package")
    try:
    ...
    if hashlib.sha256(parts["map"][1]).hexdigest() != recorded.get("map"):
        raise BacklogError(f"{nupkg}'s map is not the map provenance.json records, so it cannot say what the "
                           f"issue bodies must not quote")
...
def create(repo, engine_dir, log, gh="gh", package=None):
    ...
    compared = refuse_quotes(engine_dir, files, package)
    if compared is not None:
        print(f"--- backlog: licensed local-copy corpus (0022): {len(files)} bodies carry no text from the "
              f"corpus ({compared} quoted strings of the map compared)", file=log)
