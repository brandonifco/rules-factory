...
def packages_props(model):
    ...
    return (
        "<Project>\n\n"
        "  <!-- <auto-generated>\n"
        f"       Generated by rules-factory tools/factory from {csharp.xml_text(model.named)}.\n"
        ...
        f"{randomness}"
        "    <!-- Each map is referenced, never copied (rules-factory decision 0015), at an exact\n"
        "         version. The engine's own build facts live in overlay/. An engine composed of\n"
        "         several maps references every one of them (0067). -->\n"
        + "".join(f'    <PackageVersion Include="{package}" Version="[{version}]" />\n'
                  for package, version in model.packages)
        + "  </ItemGroup>\n\n"
        + "  <!-- The map(s) this engine is built from, referenced by the engine project only. Each\n"
        "       carries no assemblies; its build props add one RulesFactoryMap item naming the\n"
        "       restored map, manifest and checker. -->\n"
        f"  <ItemGroup Condition=\"'$(MSBuildProjectName)' == '{model.name}'\">\n"
        + "".join(f'    <PackageReference Include="{package}" PrivateAssets="all" />\n'
                  for package, _ in model.packages)
...
def refuse_split_pins(model, out):
    ...
    """
    pinned = ({"RulesKernel"} | {package for package, _ in model.packages}
              | ({RANDOMNESS_PACKAGE} if model.randomness == "seeded" else set()))
    found = []
    for directory, dirs, names in os.walk(out):
        ...
        for file_name in sorted(names):
            ...
            for element, package in re.findall(r'<(PackageVersion|PackageReference)\b[^>]*?\bInclude="([^"]+)"', text):
                ...
                elif element == "PackageReference" and (
                        package in {name for name, _ in model.packages}
                        or package.startswith("RulesFactory.Maps.")):
