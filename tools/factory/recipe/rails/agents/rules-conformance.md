...
(`tools/record-verdict.py --pr <n> --reviewer semantic --verdict pass|fail --packet <packet manifest>`). The commit comes from that packet, so it is the one you read
and not whatever the pull request has since become. If the pull request gained another commit
while you were reading, recording is refused rather than redirected, and that is the mechanism
working: review the new head or say you have not. Recording it is the whole of the step:
