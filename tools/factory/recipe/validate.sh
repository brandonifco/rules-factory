...
#     generated from, so an overlay edit that was never followed by a re-produce -- leaving a record
#     that hashes generated bytes older than the overlay they came from -- fails here. An edit
#     followed by a regeneration moves the generated hashes too, but an edit followed by nothing
#     moves only buildInputs[corpus-map.overlay.json], so that comparison is the one that catches
#     every form of it -- and a run in which it had nothing to compare fails too;
