...
| **Map validation** | [`tools/mapvalidator/`](tools/mapvalidator/__init__.py), built into `tools/check-map.py`, with [`tools/check-locators.py`](tools/check-locators.py), [`tools/check-map-review.py`](tools/check-map-review.py) and [`tools/pack-map.py`](tools/pack-map.py) | a map and its manifest | a verdict, and a map that may become a version |
