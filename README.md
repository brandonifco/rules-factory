...
| Agent rails for produced engines | this | decided ([0029](docs/decisions/0029-the-rails-are-emitted-by-default-and-vendor-choice-is-engine-owned-configuration.md), exact review identity [0049](docs/decisions/0049-a-review-verdict-is-bound-to-one-immutable-review-packet.md)); emitted and enforced: the contract, the roles, the charters, the guard, immutable review-packet manifests, dispatch, the PR contract, packet-bound recorded verdicts, `.github/agent-policy.json`, and `factory rails --apply`, which makes the three checks required. Proven on a real rule: `faa-part-107`'s `control-links-working` went issue to merge through them ([#157](https://github.com/brandonifco/rules-factory/issues/157)) |
...
rails         AGENTS.md, the roles, the three charters, the primary-checkout guard,
              dispatch, the entry and immutable review-packet manifests, the pull
              request template and its policy check, packet-bound verdict recording
              and its conformance gate, and
