...
| [0028](0028-a-pdf-text-is-derived-for-declared-printed-pages-and-marked-with-them.md) | A PDF text is derived, by a pinned pdftotext, for only the printed pages the manifest declares (`derivedText`), marked with printed page numbers under a declared PDF-page offset; `contentHash` covers the text and `sourcePdf.sha256` the PDF, and the text is never written inside a git work tree (extends 0022 and 0024) |
