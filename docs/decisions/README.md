...
| [0022](0022-a-licensed-copy-is-used-locally-by-a-named-operator-and-never-published.md) | A licensed `local-copy` corpus may be used locally by a named operator, under an explicit flag and an allowlisted `gh` identity, and a map of one is never published (extends 0013 and 0015) |
...
| [0028](0028-a-licensed-copy-map-is-restored-from-the-operators-feed-and-its-engines-ci-says-not-verified.md) | A licensed-copy map is restored from the operator's own feed (`RULES_FACTORY_LOCAL_MAP_FEED`, outside every git work tree), seeded into the NuGet global packages folder with the managed `NuGet.config` untouched; its engine's CI runs what needs no licensed input and says NOT VERIFIED, and the full verify stays on the operator's machine (extends 0022) |
