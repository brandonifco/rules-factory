...
| `tools/review-packet.py` | managed | 12 | Everything a reviewer needs about one pull request, in the order it is meant to be read. Every map the engine was produced from is read once and checked against *its own* recorded digest, matched by that digest rather than by argument order, so a composed engine is not the way an unchecked map reaches a reviewer (#460). `--role` cuts it three ways -- structural, semantic, independent -- with the whole packet still the default. The changed-path list carries the ownership class the reviewed commit's own vendored table gives each path, and the identity names the role, so a verdict can be held to the cut it was formed on (#467). The semantic cut names the files it withheld rather than counting them, and calls out `.github/agent-policy.json` when it is one of them: that file decides what is on the semantic surface, and reached the reviewer as a number (#475). The same nesting rule applies to every section it cuts to (#484). The semantic cut names the files it withheld rather than counting them, and calls out `.github/agent-policy.json` when it is one of them: that file decides what is on the semantic surface, and reached the reviewer as a number (#475). A truncated `files` list is refused, the way the conformance gate has refused one since #193: since the cuts, those paths decide what the diff contains, so a packet made from a partial list drops a change and then reports that nothing was withheld (#478). |
| `tools/repair-packet.py` | managed | 2 | The bounded brief for one repair attempt on one pull request. Everything but the findings is derived from durable state -- the current head, the branch, the worktree that holds it or the command that puts one back, the issue's acceptance criteria and required evidence, the entries the issue names, the verdicts standing at that head and what must be green. The findings are the caller's, because which ones block is the orchestrator's judgement. It carries no previous transcript, writes nothing, and prints to stdout (#465). A section of the issue keeps what is nested under it: a `###` inside `## Acceptance criteria` used to end the section and take every criterion with it (#484). |
