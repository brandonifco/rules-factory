...
| `tools/orchestrator-status.py` | managed | 2 | Where the work on this engine stands, read from git and GitHub and bounded on purpose: the checkout and whether it is clean and current, every worktree with the issue its branch names, every open pull request with its head, its checks and the verdicts recorded at it, and the open issues by the state label the policy names. No issue body, no pull request body, no diff, no log -- it is small enough to be the context a fresh orchestrator starts from, which is the point. Read-only, storing nothing, and `NOT CHECKED` wherever it could not read rather than a repository it never looked at (#466). Read-only of the index as well as of the repository: an ordinary `git status` refreshes and rewrites `.git/index` by default and takes its lock to do it, in a checkout another agent may be committing in (#479). |
