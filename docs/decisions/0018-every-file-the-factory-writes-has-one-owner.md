...
| `tools/review-packet.py` | managed | 11 | Everything a reviewer needs about one pull request, in the order it is meant to be read. Every map the engine was produced from is read once and checked against *its own* recorded digest, matched by that digest rather than by argument order, so a composed engine is not the way an unchecked map reaches a reviewer (#460). `--role` cuts it three ways -- structural, semantic, independent -- with the whole packet still the default. The changed-path list carries the ownership class the reviewed commit's own vendored table gives each path, and the identity names the role, so a verdict can be held to the cut it was formed on (#467). The semantic cut names the files it withheld rather than counting them, and calls out `.github/agent-policy.json` when it is one of them: that file decides what is on the semantic surface, and reached the reviewer as a number (#475). The same nesting rule applies to every section it cuts to (#484). |
