...
| `tools/review-packet.py` | managed | 4 | Everything a reviewer needs about one pull request, in the order it is meant to be read, every byte of it read from the one commit under review. |
...
| `tools/record-verdict.py` | managed | 2 | A review verdict as a commit status on the exact commit reviewed, named by the reviewer or by the packet they read, so a later commit invalidates it by itself and never inherits it. |
