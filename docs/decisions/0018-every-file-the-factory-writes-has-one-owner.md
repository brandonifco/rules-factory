...
| `.claude/agents/repo-steward.md` | managed | 2 | The structural reviewer's charter, read-only, and carrying the `model:` line that makes the "Cheap" in its own description true (#455). |
| `.claude/agents/rules-conformance.md` | managed | 5 | The semantic reviewer's charter, read-only, on the deepest tier: the role not to economise on (#455). |
...
| `tools/review-packet.py` | managed | 8 | Everything a reviewer needs about one pull request, in the order it is meant to be read. Every map the engine was produced from is read once and checked against *its own* recorded digest, matched by that digest rather than by argument order, so a composed engine is not the way an unchecked map reaches a reviewer (#460). |
...
| `tools/record-verdict.py` | managed | 4 | A review verdict as a commit status on the exact commit reviewed, so a later commit invalidates it by itself. The identity names every map, and every one of them must be bound to the reviewed commit: one unbound map among several is the whole failure, not a fraction of it (#460). |
