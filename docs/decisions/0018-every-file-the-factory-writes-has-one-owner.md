...
| `tools/review-packet.py` | managed | 9 | Everything a reviewer needs about one pull request, in the order it is meant to be read. Every map the engine was produced from is read once and checked against *its own* recorded digest, matched by that digest rather than by argument order, so a composed engine is not the way an unchecked map reaches a reviewer (#460). `--role` cuts it three ways -- structural, semantic, independent -- with the whole packet still the default. The changed-path list carries the ownership class the reviewed commit's own vendored table gives each path, and the identity names the role, so a verdict can be held to the cut it was formed on (#467). |
