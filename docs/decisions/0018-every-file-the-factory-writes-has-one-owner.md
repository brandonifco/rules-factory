...
| `tools/entry-packet.py` | managed | 6 | The bounded assignment for one entry, assembled from merge(package, overlay) so it cannot carry a reading of its own. |
...
| `tools/review-packet.py` | managed | 7 | Everything a reviewer needs about one pull request, in the order it is meant to be read. |
