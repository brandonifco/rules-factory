...
| `AGENTS.md` | managed | 23 | The governing contract every agent works the engine under (0029). Managed, not generated: a team may amend its own contract, and the factory must then either carry the amendment or refuse and say so, never silently overwrite it. Section 4 carries the leftover sweep, the `## Documentation` section every pull request answers, and the rule to delete only what you created, by its exact path (#236). The review section says that changing the risk label on the linked issue asks the gate again, whenever it is changed (#230). Section 11 is what the machine owes the work rather than what the engine does: the SDK `global.json` pins, a restore before the first packet, and a `gh` new enough for the field the packets read (#195). Section 9 names the two emitted things that do name a vendor -- the bundled Claude adapter and the policy's default review chain -- as the engine-owned defaults they are, where it used to say that nothing emitted named one (#174). Section 7 says how a recorded mutation is run, and that `tools/mutate.py` reports the one failure the prose cannot -- a mutation that leaves its test green (#454). Section 7 also says that a composed engine needs one `--package-map` per package, that the entry id names the package an entry came from, and that an entry another package supersedes is not work (#460). Section 4 ends an implementation attempt at the review boundary: one instance is one attempt, a blocking finding starts a fresh one on the same issue, branch and worktree, and `tools/repair-packet.py` is the brief it reads (#465). Section 3 says what the orchestrator holds -- the current scheduling decision -- and what the repository holds, and names `tools/orchestrator-status.py` as the recovery path (#466). Section 7 says a reviewer is given the context its role judges rather than the whole of it, and that the role is part of the packet's identity (#467). |
...
| `docs/agent-team.md` | managed | 9 | The four roles and what each may not do (0029), and what a turn costs: an agent's cost is its turns multiplied by its context, so the structural review runs on the cheaper tier and the semantic review on the deepest available. Stated without naming any model -- the tier is the contract's, the model is the vendor adapter's (0029's amendment, #455). The orchestrator holds the current scheduling decision and reads the rest, which is what makes a fresh one cheap (#466). Each reviewer's section names the cut of the packet it reads, and what that cut leaves out (#467). |
| `.claude/agents/engine-dev.md` | managed | 10 | The implementer's charter. Section "What you do" names `tools/mutate.py` as how a recorded mutation is observed, rather than leaving the loop to be rebuilt per implementer (#454). It ends with the attempt rather than outliving the review, and says what a repair attempt reads and what it may not reconstruct (#465). |
| `.claude/agents/repo-steward.md` | managed | 3 | The structural reviewer's charter, read-only, and carrying the `model:` line that makes the "Cheap" in its own description true (#455). It reads the structural cut of the packet: the claim in full and the changed paths with their ownership class, no entry packet, and therefore no restored map package needed to make one (#467). |
| `.claude/agents/rules-conformance.md` | managed | 6 | The semantic reviewer's charter, read-only, on the deepest tier: the role not to economise on (#455). It reads the semantic cut: the entry packets first, and not the implementer's case for its own reading of the rule (#467). |
...
| `tools/entry-packet.py` | managed | 7 | The bounded assignment for one entry, assembled from merge(package, overlay) so it cannot carry a reading of its own. An engine composed of several map packages is read as one: the model is built over the whole composition, the entry id's prefix says which package the entry came from and the header names that package at its own version, and an entry another map supersedes says so instead of being handed out as work (#460). |
| `tools/mutate.py` | managed | 1 | A recorded mutation, run: the edits applied, the named test run alone, the source restored in a `finally`. The mutation AGENTS.md §7 requires becomes a spec a reviewer can re-run rather than a sentence, and a mutation that leaves its test green -- the test nobody has watched fail -- is reported and exits non-zero (#454). |
| `tools/re-produce.sh` | managed | 6 | Re-runs `factory produce` on the engine from the factory commit `provenance.json` names. The record is generated, so an overlay edit is only finished by a produce, and prose describing that clone is a procedure an operator can get wrong (#192). It also recognises a record a merge left conflicted, where no side is the right one because every hash in it is about to be recomputed, and `--resolve-record` takes a side and re-produces over it (#252). |
| `tools/review-packet.py` | managed | 12 | Everything a reviewer needs about one pull request, in the order it is meant to be read. Every map the engine was produced from is read once and checked against *its own* recorded digest, matched by that digest rather than by argument order, so a composed engine is not the way an unchecked map reaches a reviewer (#460). `--role` cuts it three ways -- structural, semantic, independent -- with the whole packet still the default. The changed-path list carries the ownership class the reviewed commit's own vendored table gives each path, and the identity names the role, so a verdict can be held to the cut it was formed on (#467). The semantic cut names the files it withheld rather than counting them, and calls out `.github/agent-policy.json` when it is one of them: that file decides what is on the semantic surface, and reached the reviewer as a number (#475). The same nesting rule applies to every section it cuts to (#484). A truncated `files` list is refused, the way the conformance gate has refused one since #193: since the cuts, those paths decide what the diff contains, so a packet made from a partial list drops a change and then reports that nothing was withheld (#478). |
| `tools/repair-packet.py` | managed | 3 | The bounded brief for one repair attempt on one pull request. Everything but the findings is derived from durable state -- the current head, the branch, the worktree that holds it or the command that puts one back, the issue's acceptance criteria and required evidence, the entries the issue names, the verdicts standing at that head and what must be green. The findings are the caller's, because which ones block is the orchestrator's judgement. It carries no previous transcript, writes nothing, and prints to stdout (#465). A section of the issue keeps what is nested under it: a `###` inside `## Acceptance criteria` used to end the section and take every criterion with it (#484). A repair attempt is an implementation attempt, so it makes the readiness refusal `tools/dispatch-agent.sh` makes -- an issue awaiting a decision, blocked or closed gets no brief -- and it says a semantic verdict is owed only when the changed paths owe one, which is what the review packet and the conformance gate decide from (#483). |
| `tools/pr-policy.py` | managed | 8 | The pull request contract, checked mechanically: one linked issue, every section filled, output rather than a claim. A `factory produce` update's claim is checked against `provenance.json` and this table, never taken (#193). A retired path counts only as a deletion the base commit's record attributes to the factory (#243). Every document the engine owns is accounted for, and every `*.md` the diff touches is listed as updated (#236). The entry ids declared by the pull request, the linked issue's entry marker and the overlay entries moved to `implemented` must identify the same work (#451). |
| `tools/record-verdict.py` | managed | 5 | A review verdict as a commit status on the exact commit reviewed, so a later commit invalidates it by itself. The identity names every map, and every one of them must be bound to the reviewed commit: one unbound map among several is the whole failure, not a fraction of it (#460). It also refuses a verdict the packet's role could not carry: a structural cut holds no entry evidence at all, and a semantic verdict formed on one would be the unbound evidence #372 refuses, arriving through the role rather than through the digest (#467). |
...
| `tools/requeue-gate.py` | managed | 2 | Asks the gate to report again when something outside the pull request changed what it would answer: a verdict recorded at its head (#191), or the risk label on the issue it closes (#230). Recording either is then the whole of the step. It writes no status and no check run of its own. |
| `.github/pull_request_template.md` | managed | 5 | The pull request shape `pr-policy.py` checks, its `## Documentation` section included (#236). Template and checker are emitted together, so neither can drift from the other. `## Unrelated changes` says what belongs in it, and that a document the change makes untrue is part of the change rather than an unrelated one (#196). The entry list says that `pr-policy.py` compares it with the linked issue marker and overlay transition (#451). |
...
| `.github/workflows/verdict-requeue.yml` | managed | 2 | Runs `requeue-gate.py` on the `status` and `issues` events. Deliberately not a required check: it runs on the default branch's commit, where a required check governs nothing. |
| `tools/agent-doctor.py` | managed | 6 | Whether the rails are active or only present: the rails byte for byte as the recipe wrote them, the hook wired, the policy one a verdict can be recorded under, the labels created, and the checks required, pinned to the app that posts them, and set by a ruleset of this repository's own (#211, #231). One row spans both halves: what merged work left behind, `NOT CHECKED` rather than `OK` wherever it could not ask (#236). Before any of them, three rows about the **machine**: the pinned SDK, whether anything is restored, and whether `gh` has the field the packets ask it for -- every rail can be in place on a machine that cannot run the gate, and the first live run lost an afternoon to exactly that (#195). |
| `tools/orchestrator-status.py` | managed | 3 | Where the work on this engine stands, read from git and GitHub and bounded on purpose: the checkout and whether it is clean and current, every worktree with the issue its branch names, every open pull request with its head, its checks and the verdicts recorded at it, and the open issues by the state label the policy names. No issue body, no pull request body, no diff, no log -- it is small enough to be the context a fresh orchestrator starts from, which is the point. Read-only, storing nothing, and `NOT CHECKED` wherever it could not read rather than a repository it never looked at (#466). Read-only of the index as well as of the repository: an ordinary `git status` refreshes and rewrites `.git/index` by default and takes its lock to do it, in a checkout another agent may be committing in (#479). And three answers it used to give without establishing them: a list GitHub cut is reported as cut rather than counted, a worktree it could not read or could not reach is unknown rather than absent, and exit 0 means the steady state the contract defines -- main, clean, level with the default branch -- rather than the first of those three (#480). |
