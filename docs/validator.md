...
| [`tools/check-locators.py`](../tools/check-locators.py) and the two per-grammar checkers under `examples/` | the evidentiary half: does the cited text exist where the map says, does no in-scope quote lie outside the declared extent (`extent-bounds`, [#269](https://github.com/brandonifco/rules-factory/issues/269)), and how much of that extent can the map show it quoted ([0054](decisions/0054-coverage-reports-how-much-of-the-extent-is-quoted-and-a-map-declares-the-floor.md)) |
