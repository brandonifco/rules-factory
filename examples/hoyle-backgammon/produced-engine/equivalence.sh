...
# the factory at [factory ref] (default factory/v0.2.1) in a scratch git worktree, and requires that
...
if [ "$#" -lt 1 ] || [ "$#" -gt 2 ]; then
  echo "usage: $0 <engine sha> [factory ref, default factory/v0.2.1]" >&2
  exit 2
fi
ENGINE_SHA="$1"
FACTORY_REF="${2:-factory/v0.2.1}"
