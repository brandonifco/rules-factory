...
{ "schemaVersion": 1, "corpus": "cfr-14-107",
  "baseline": { "contentHash": "80f6bc4b…", "hashDerivation": "ecfr-versioner-xml",
                "asOf": "2026-01-01" },
  "extent": { "unit": "page", "from": 12, "to": 19 },
  "entries": [ … ] }
```
...
read, which deriving the extent from the citations would have allowed: a map whose citations
stop short of the last pages of a chapter would silently shrink its extent past whatever those
pages state.
...
decided in decision 0010: ten of the
...
`opening-advice`'s "lead your strong suit whenever you **reasonably** can" (an invented card game) is an undefined degree that demands
...
1. **A blank in the rule, or a fact the rule tests?** Does applying the corpus's words require
   ...
   entry of its own**. Applying *"either two or three tokens"* requires fixing the forfeit;
   applying *"either wholly from the stock, wholly from the discard pile, or partly from each"*
   requires fixing nothing, because the corpus states every branch (both invented card-game
   examples). Whether ATC authorized
   is a fact the rule tests, not a blank. **"Or case" is load-bearing and is there for two
   instances**: an invented `hand-scoring` whose named results do not cover a reachable tie,
   and an invented `must-follow-suit` that does not say which card must be played when two are
   equally legal. Neither is an
...
2. **Unsupplied by the corpus, or only to us?** Readable but not by this adapter →
   ...
   differently → a conflict under decision 0007.
...
to create a collision hazard**" — or the fixed set: "**either two or three tokens**". An
...
not be **named**: an agentless passive such as *"(as may have been agreed)"* and names nobody, which is
...
decision 0010 rules that being the only
...
`evidence` held a summary — "Both suits.", "Both elections." — nothing was findable, so
nothing was checkable, and **many of one trial map's citations were wrong by a page or
two through a mapping trial, a build and a review**. The wrong pages are not the
...
check reporting a pass over a fraction of what it appears to have read. One trial
entry was exactly this: a long `evidence` of which only its first few words were ever checked. Truncating a
...
**A span may straddle a page marker, and carries it verbatim.** A sentence may begin
on p. 12 with the `{13}` marker falling mid-sentence. Citing either page is honest and the
checker accepts both; dropping the marker to make the quote read cleanly would break the match.

**What must be demonstrated is a mapper's reading, and it goes in `note`.** "Both tie directions",
"every row of the scoring table", "a lead that can follow suit, can trump, and can do neither" — these are genuinely useful and no check can read them. They are
...
an invented `opening-advice` quotes the opening sentence of the advice it declines, and demands
nothing of the engine. A rule the corpus does not state at all — an invented `turn-timer`, which
a printed card-game text predates — quotes **the passage the rule would be in**, and carries `absentFrom` to say that is
what the span is. There is no exception and no entry without a span. Until
decision 0009 there was: the citation read
`(absent)`, the checker special-cased it, and the gate's line reporting every entry
checked meant one fewer.
...
An invented `discard-rule` permits only a face-up discard where `discard-pile`, a paragraph
earlier, permits face-up or face-down. The `question` states both readings; `fate` records which governs, or
declines.
...
"ambiguity": {
  "question": "…",
  "conflict": "orientation-of-a-discard",
  "fate": "decision",
  "decision": "docs/decisions/0098-discard-orientation.md"
}
```

**A conflict is a question, not a pair.** Suppose three entries carry this slug and do
not each contradict each other — `discard-rule` and `hand-reveal` fit together
exactly, and both disagree with `discard-pile`. What makes them one conflict is that they
are three answers to *how a discarded card is placed*. A list of pairwise
...
`dependsOn` orders *implementation*. A rule that applies only in a phase — melding
begins once a player has drawn; a pending penalty draw suspends every other play — has a
...
other. In an invented card-game map, `final-round-scoring` may have several `dependsOn` ancestors and
exactly one of them its gate; `play-card` may be gated by `penalty-draw`, which is not
among its ancestors, and nor is `play-card` among `penalty-draw`'s. A stateless corpus
...
It says nothing about *direction*. `final-round` permits the entries it gates;
`penalty-draw` suspends them. Only the referenced entry's text distinguishes those, and a
...
`modality` is a short noun phrase naming what holds the rule — `illustration`, for example.
Deliberately not a closed vocabulary: it can be closed later from evidence. Explanation goes in `note`, not here.

`locator` is still required and still cites the passage that *states* the rule — the
sentence saying the cards are laid out as in Diagram A. The entry is unreadable, not uncitable.
...
The general case is worse than an illustration: a PDF rulebook read as extracted text
...
```json
"absentFrom": { "searched": ["timer", "time limit", "turn clock", "seconds"] }
```
...
search is bounded by `extent` and must be: a term may be absent from the chapter a map covers
and occur elsewhere in the same volume, so a whole-volume search would refuse a
...
passage the rule would be in** — for `turn-timer`, the sentence that sets out how a turn
proceeds and ends. So an absence costs a mapper *more* reading than a scope
...
An invented instance: a card-game map excludes *Notes on Play* wholesale as advice, and inside it
sits, in a worked example, the only statement in the corpus of the hand size.
`opening-advice` declines **the advice**; `hand-size` is `scope: in` and cites the same
...
```json
"crossReferences": [
  { "cites": "as at the deal", "resolvedBy": "dealing" },
  { "cites": "as in Diagram A", "unmapped": "Diagram A is an illustration, not a passage." }
]
```
...
**Two limits.** The phrase list is closed and short: an entry quoting *"as shown in
{13} Diagram A"* and the page marker falling inside the phrase hides it, so a corpus that points
...
case*. An invented `must-follow-suit` is the instance — the engine implements the compulsion and
declines only where two legal plays are equally preferred, which is neither `declined` nor a
...
entry that declines one shape of input from one that declines everything. The totality claim
is weaker than it was.

**An entry whose correct reading is surprising names the test that proves it.** Not a field —
a flag saying "this is surprising" is unfalsifiable. An invented `trick-winner` could be clear, correct,
...
carrying `fate: unresolved` matches two rows — several entries across the existing maps —
...
corpus *did* state, which is what reading a delegated choice between two stated values as an ambiguity costs.
...
- #5 — `dependsOn` conflates
  ...
  decision 0003 adds `gatedBy`.
- #6 — a standard is not a gap.
  Decided: decision 0005 — a delegated
...
- #20 — `scope: out` applied to a
  section cost a map a rule stated inside the excluded section. Decided:
  decision 0009 — `scope` is per rule, and
  `extent` coverage is what records that a section was read.
- #28 — a carve-out with no entry in
  either direction. Decided: decision 0009 adds
...
- #11 — nothing used
  ...
  Decided: decision 0010 — whose fact
...
decision 0010 closes the other half of
