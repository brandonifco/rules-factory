...
modality. The initial layout of an invented card game might be given entirely
...
the entry that a tool or a reviewer can hold against the page. In one trial many of a
...
In an invented card game: four suits named and ranked, ranks running two through ace, "the
eldest hand being the player on the dealer's left" — these read as preamble and are
load-bearing. Without them the entries that use the vocabulary cannot be interpreted at all, and
nothing else in the map says a suit has thirteen ranks or who the eldest hand is. A mapper
hunting for rules reads past vocabulary, and misses exactly these.
...
generally wise to hold back the highest card" might sit in the middle of a card game's play rules. Drop
...
structure has rules that only apply in a phase: melding begins once a player has drawn;
a pending penalty draw suspends every other play. That is a different fact from implementation
...
decision 0010 this section also listed
...
states. A printed card-game text that predates timed play has no turn timer; an engine built from it
is an engine of that text, and that is a correct outcome rather than an incomplete one.
...
decision 0009 the map spelled all three the
...
first.** A section is a unit of the corpus's layout; `scope` is a judgement about a rule. A map
that excludes a section wholesale as advice loses any rule stated inside it. Two entries citing one section
...
two or three tokens (as may have been agreed)"*, in an invented card game, is the clearest case —
read as an ambiguity, the engine discards the rule that the forfeit is two or three and nothing else.
...
1. **A blank in the rule, or a fact the rule tests?** Does applying the corpus's words require
   ...
   rule with a caller-supplied parameter, and the parameter gets no entry. *"Either two or three
   tokens"* leaves the forfeit unfixed and is a blank; *"either wholly from the stock, wholly from
   the discard pile, or partly from each"* states every branch and is not (both invented). "Or case"
   is what catches a defective enumeration — scoring that names no result for a reachable tie —
   which is a gap without being an open degree.
...
decision 0010, which closes the half of
