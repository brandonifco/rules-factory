...
| 10 | [49 CFR § 172.101–§ 172.102](hazmat-172-table/) — the Hazardous Materials Table and the special provisions, 2026-01-01 | Rules stated as a grid, pointers that are codes rather than words, applicability encoded in the shape of an identifier, and a specific provision governing a general one | **admitted, not yet mapped** | — | Nothing yet: this is phase 1 only. The corpus is pinned (2.9 MB committed whole, because any reduction would be a mapper-chosen extraction of a table), the slice is settled — seven rows of table 3, § 172.101(a) and (b)–(j), § 172.102(a), (b)(1)–(9) and the 20 provisions those rows invoke — and [the hypotheses H1–H6 are written down before the map exists](hazmat-172-table/README.md#the-hypotheses-before-anything-is-mapped), which is what [#262](https://github.com/brandonifco/rules-factory/issues/262) is for. Verified at admission: all seven rows resolve from a `column 2` key to exactly one row, `9A`/`9B` on Acetal are `5 L` and `60 L`, and the 20 codes are the 20 #262 settled on. Two findings filed: [#284](https://github.com/brandonifco/rules-factory/issues/284), the protocol's `pointerMechanisms` has no value for a column 7 code and `adapterReach` no modality for an equation — H2's evidence, recorded and deliberately not fixed under [#265](https://github.com/brandonifco/rules-factory/issues/265); and [#285](https://github.com/brandonifco/rules-factory/issues/285), 12 of the 20 provisions are stated inside an `<EXTRACT>`, which [0035](../docs/decisions/0035-a-rule-stated-in-a-table-row-is-cited-by-its-row.md) did not reach — since closed by [0036](../docs/decisions/0036-a-paragraph-inside-a-wrapper-takes-the-designation-the-wrapper-continues.md), so all 20 are citable: the eight table-stated codes by their rows and the twelve others at the designation their run continues, `§ 172.102(c)(1)` through `(c)(9)`. § 172.102 now indexes 94.8% of its characters rather than 5.7%, and § 172.101's two appendices are reported **unplaced** rather than attributed to the shipping-name rule they are printed after, while its note gains the address it prints for itself, `§ 172.101(c)(11)`. Two further findings are filed and not fixed: [#289](https://github.com/brandonifco/rules-factory/issues/289), the designator expression requires whitespace after the token, and [#290](https://github.com/brandonifco/rules-factory/issues/290), what the walk still passes over outside a wrapper. Numbered for its issue; the validator-attack run below also calls itself trial 10, which is [#286](https://github.com/brandonifco/rules-factory/issues/286) |
