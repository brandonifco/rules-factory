...
| `inputs/factory/GET-FACTORY.sh` | Checks out the factory at the tag you must produce with, tools only. |
...
2. **Do not look at rules-factory beyond what this brief gives you.** In particular not
   ...
   issues and pull requests (#3 and its links discuss the target). The one network fetch of
   rules-factory allowed is `inputs/factory/GET-FACTORY.sh`.
...
4. Network access is allowed for nuget.org restores, the .NET SDK, and documentation of the kernel,
   .NET and xUnit. Nothing else about backgammon engines: no other backgammon implementation.
5. If something you need is not here, **ask in writing**. The answer will be given in writing, added to
   this brief as a disclosure, and counted. Do not guess from outside sources.
...
6. The rules above were kept: your transcript or reading log is searched, and every question you asked
   is published with its answer.
